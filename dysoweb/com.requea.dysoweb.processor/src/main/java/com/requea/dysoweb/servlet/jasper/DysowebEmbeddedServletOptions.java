...
import com.requea.dysoweb.jasper.Options;
 ...
 */
public final class DysowebEmbeddedServletOptions implements Options {
    
    // Logger
    private final Log log = LogFactory.getLog(DysowebEmbeddedServletOptions.class);
     ...
     */
    public DysowebEmbeddedServletOptions(ServletConfig config,
            ServletContext context) {
        
    	if(config instanceof RequestProcessor.JasperConfig) {
    		fBundle = ((RequestProcessor.JasperConfig)config).getBundle();
    	}
         ...
         * targetdir
         */
        // check if we should use bundle directory
        if(false && fBundle != null && fBundle.getLocation().startsWith("reference:file:")) {
        	// bundle is read / write: get the location and check that resources dir exists
        	File baseDir = new File(fBundle.getLocation().substring("reference:file:".length()));
        	File resourcesDir = new File(baseDir, "resources");
        	if(resourcesDir.exists() && resourcesDir.canRead() &&
        			resourcesDir.canWrite() && resourcesDir.isDirectory()) {
        ...
        // web-application.
        tldLocationsCache = new DysowebTldLocationCache(context);
