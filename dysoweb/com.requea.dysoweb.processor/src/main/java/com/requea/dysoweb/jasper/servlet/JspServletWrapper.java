...
package com.requea.dysoweb.jasper.servlet;
...
import com.requea.dysoweb.jasper.compiler.ErrorDispatcher;
import com.requea.dysoweb.jasper.compiler.JarResource;
import com.requea.dysoweb.jasper.compiler.JavacErrorDetail;
...
public class JspServletWrapper {
    ...
    public void service(HttpServletRequest request, 
        ...
        try {
             ...
             */
            // try to get the servlet if not dev
            if(firstTime && options.getDevelopment() == false && options instanceof DysowebEmbeddedServletOptions) {
                try {
                	DysowebEmbeddedServletOptions dysoweboptions = (DysowebEmbeddedServletOptions)options;
                	Bundle bundle = dysoweboptions.getBundle();
                	Class<?> clazz = bundle.loadClass(ctxt.getFQCN());
                	if(clazz != null) {
                        InstanceManager instanceManager = InstanceManagerFactory.getInstanceManager(config);
                        servlet = (Servlet) instanceManager.newInstance(ctxt.getFQCN(), clazz.getClassLoader());
                        servlet.init(config);
                        theServlet = servlet;
                        reload = false;
                        firstTime = false;
