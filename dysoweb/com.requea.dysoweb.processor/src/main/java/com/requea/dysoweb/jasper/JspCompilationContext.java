...
import com.requea.dysoweb.jasper.compiler.JarResource;
import com.requea.dysoweb.jasper.compiler.JspRuntimeContext;
import com.requea.dysoweb.jasper.compiler.JspUtil;
import com.requea.dysoweb.jasper.compiler.Localizer;
import com.requea.dysoweb.jasper.compiler.ServletWriter;
import com.requea.dysoweb.jasper.compiler.TldLocation;
import com.requea.dysoweb.jasper.servlet.JasperLoader;
...
public class JspCompilationContext {
    ...
    protected String outputJavaDir;
    protected String outputClassDir;
    ...
    public ClassLoader getJspLoader() {
        if( jspLoader == null ) {
            File base = options.getTargetDir();
            URL targetUrl;
            try {
				targetUrl = base.toURI().toURL();
			} catch (MalformedURLException e) {
				// ignore
				targetUrl = baseUrl;
			}
            jspLoader = new JasperLoader
            (this.basePackageName, new URL[] { targetUrl },
                    getClassLoader(),
     ...
     */
    public String getOutputClassDir() {
        if (outputClassDir == null) {
            createOutputClassDir();
        }

        return outputClassDir;
    }
    ...
    public Compiler createCompiler() {
        ...
        if (options.getCompilerClassName() != null) {
            ...
            if (options.getCompiler() == null) {
                ...
                if (jspCompiler == null) {
                    jspCompiler = createCompiler("com.requea.dysoweb.jasper.compiler.AntCompiler");
                }
            } else {
                jspCompiler = createCompiler("com.requea.dysoweb.jasper.compiler.AntCompiler");
                if (jspCompiler == null) {
    ...
    public String getServletJavaFileName() {
        if (servletJavaFileName == null) {
            servletJavaFileName = getOutputJavaDir() + getServletClassName() + ".java";
        }
    ...
    public String getClassFileName() {
        if (classFileName == null) {
            classFileName = getOutputClassDir() + getServletClassName() + ".class";
        }
    ...
    public void checkOutputDir() {
        if (outputJavaDir != null) {
            if (!(new File(outputJavaDir)).exists()) {
                makeOutputJavaDir();
            }
        } else {
            createOutputJavaDir();
        ...
        if (outputClassDir != null) {
            if (!(new File(outputClassDir)).exists()) {
                makeOutputClassDir();
            }
        } else {
            createOutputClassDir();
    ...
    protected boolean makeOutputJavaDir() {
        synchronized(outputDirLock) {
            File outDirFile = new File(outputJavaDir);
            return (outDirFile.mkdirs() || outDirFile.isDirectory());
        }
    }
    protected boolean makeOutputClassDir() {
        synchronized(outputDirLock) {
            File outDirFile = new File(outputClassDir);
            return (outDirFile.mkdirs() || outDirFile.isDirectory());
        }
    }

    protected void createOutputJavaDir() {
        String path = null;
        if (isTagFile()) {
            String tagName = tagInfo.getTagClassName();
        ...
        try {
            ...
            outputJavaDir = base.getAbsolutePath() + File.separator + path +
                File.separator;
            if (!makeOutputJavaDir()) {
    ...
    protected void createOutputClassDir() {
        String path = null;
        if (isTagFile()) {
            String tagName = tagInfo.getTagClassName();
            path = tagName.replace('.', File.separatorChar);
        ...
        try {
            File base = options.getTargetDir();
            baseUrl = base.toURI().toURL();
            outputClassDir = base.getAbsolutePath() + File.separator + path +
                File.separator;
            if (!makeOutputClassDir()) {
                throw new IllegalStateException(Localizer.getMessage("jsp.error.outputfolder"));
            }
        } catch (MalformedURLException e) {
            throw new IllegalStateException(Localizer.getMessage("jsp.error.outputfolder"), e);
